...
$request_type_labels = [
    ''          => __('Всі типи', 'wp-suspicious-activity'),
    'page'      => __('Сторінка', 'wp-suspicious-activity'),
    'api'       => __('API', 'wp-suspicious-activity'),
    'admin'     => __('Адмінка', 'wp-suspicious-activity'),
    'heartbeat' => __('Heartbeat', 'wp-suspicious-activity'),
];
?>
<div class="wrap wp-sad-wrapper">
    <h1 class="wp-heading-inline"><?php esc_html_e('Лог запитів', 'wp-suspicious-activity'); ?></h1>
    ...
    <form method="get" class="wp-sad-filters" data-sad-filters-form style="margin-top: 20px;">
        ...
        <div class="wp-sad-filter-group">
            <label for="sad-device"><?php esc_html_e('Пристрій (User-Agent містить):', 'wp-suspicious-activity'); ?></label>
            <input type="text" id="sad-device" name="device" value="<?php echo esc_attr($filters_view['device']); ?>" placeholder="Chrome, iPhone...">
        </div>

        <div class="wp-sad-filter-group">
            <label for="sad-request-type"><?php esc_html_e('Тип запиту:', 'wp-suspicious-activity'); ?></label>
    ...
    <?php if (empty($results_view)): ?>
        <div class="wp-sad-empty">
            <strong><?php esc_html_e('За вказаними критеріями записів не знайдено.', 'wp-suspicious-activity'); ?></strong>
        </div>
    <?php else: ?>
        <p style="margin: 10px 0; color: #666;">
            <?php esc_html_e('Знайдено записів:', 'wp-suspicious-activity'); ?> <strong><?php echo esc_html(number_format_i18n($total_view)); ?></strong>
        </p>
        ...
        <table class="wp-sad-table">
            <thead>
                <tr>
                    <?php
                    echo WP_SAD_View_Helpers::render_sortable_header('created_at', __('Час', 'wp-suspicious-activity'), $sort_view, $query_args, $base_url);
                    echo WP_SAD_View_Helpers::render_sortable_header('user_email', __('Користувач', 'wp-suspicious-activity'), $sort_view, $query_args, $base_url);
                    echo WP_SAD_View_Helpers::render_sortable_header('ip', __('IP', 'wp-suspicious-activity'), $sort_view, $query_args, $base_url);
                    ?>
                    <th><?php esc_html_e('Пристрій', 'wp-suspicious-activity'); ?></th>
                    <?php echo WP_SAD_View_Helpers::render_sortable_header('request_type', __('Тип', 'wp-suspicious-activity'), $sort_view, $query_args, $base_url); ?>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td><?php echo WP_SAD_View_Helpers::render_user_link($row->user_id, $row->user_email, $row->display_name); ?></td>
                        ...
                        <td><a href="<?php echo esc_url($view_url); ?>" class="button button-small button-view-logs" target="_blank"><?php esc_html_e('Переглянути', 'wp-suspicious-activity'); ?></a></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
